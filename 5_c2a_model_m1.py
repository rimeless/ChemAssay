...
class Chem2AssayDataset(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        if prot in self.aid2seq.index:
            target_seq=self.aid2seq.loc[prot,"SEQ"]
        else:
            target_seq=None
...
def train(args):
    ...
    model=Chem2AssayModel(args.chemberta,args.assay_model,args.prot_model,args.temp,args.lambda_bce,args.use_infonce).to(device)
    ...
    if dist.get_rank()==0:
        ...
        if not os.path.exists(csv_path):
            with open(csv_path,"w") as cf:
                cf.write("epoch,loss,bce,infonce\n")
    for e in range(1,args.epochs+1):
        ...
        for b in loader:
            ...
            scaler.update()
            tot_loss+=loss.item()
            tot_bce+=out["loss_bce"].item()
            if out["loss_infonce"] is not None:
                tot_infonce+=out["loss_infonce"].item()
                infon_steps+=1
            cnt+=1
        if dist.get_rank()==0:
            avg_loss=tot_loss/max(1,cnt)
            avg_bce=tot_bce/max(1,cnt)
            ...
            with open(csv_path,"a") as cf:
                cf.write(f"{e},{avg_loss},{avg_bce},{avg_infonce}\n")
    ...
    if dist.get_rank()==0 and log_f is not None:
        log_f.close()
...
def parse():
    ...
    p.add_argument("--out_dir",type=str,default="./ckpts")
...
# CUDA_VISIBLE_DEVICES=0,1,3,6 torchrun --nproc_per_node=2 --master_port=29501 train_m2.py \
...
# CUDA_VISIBLE_DEVICES=3,6 torchrun --nproc_per_node=2  --master_port=29501 train_m1.py \
#   --biodf2 sub_biodf2.pkl \
#   --aid2seq alldf_pivot.pkl \
#   --apdf pubchem_ap_opt.pkl \
#   --smidf pubchem_smiles_opt.pkl \
#   --out_dir ./ckpts_exp1 \
...
# CUDA_VISIBLE_DEVICES=4,5 torchrun --nproc_per_node=2 --master_port=29501 train.py ...
